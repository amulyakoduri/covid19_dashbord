...
class StateTotalData extends Component {
  ...
  getEachState = async () => {
    ...
    const totalConfirmedCases = eachStateTotalData.confirmed

    const totalRecoveredCases = eachStateTotalData.recovered

    const totalDeceasedCases = eachStateTotalData.deceased
  ...
  render() {
    ...
    const isActiveTab = active ? 'confirmed-block' : ''
    return (
      <>
        <ul className="ul-list-each-state ">
          <li
            className={`category-item ${confirmedData.name} ${isActiveTab}`}
            tabIndex="-1"
